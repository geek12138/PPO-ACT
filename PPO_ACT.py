...
class ActorCritic(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=64):  # 增大隐藏层维度
        super().__init__()
...
class SPGG(nn.Module):
    def __init__(self, L_num, device, alpha, gamma, clip_epsilon, r, epoches, 
    ...
    def ppo_update(self):
        ...
        with torch.no_grad():
            _, values = self.policy(states,self.epoch)
            _, next_values = self.policy(next_states,self.epoch)
        ...
        for _ in range(self.ppo_epochs):
            for batch in self._make_batch(states, actions, old_log_probs, advantages, returns):
                ...
                if ret_b.shape[0]==1:
                    ret_b=ret_b.squeeze()
                probs, value_pred = self.policy(state_b,self.epoch)
    ...
    def run(self, num, start_epoch=0):
        coop_rates = []
        defect_rates = []
        total_values = []
        ...
        for epoch in tqdm(range(self.epoches)):
            self.epoch=epoch
            total_epoch=epoch+start_epoch-1
            # epoch_save=epoch+start_epoch-1
            action, log_prob = self.choose_action(self.current_state,self.epoch)
            ...
            if epoch==0:
                ...
                asyncio.create_task(self.shot_pic(self.current_state, total_epoch, self.r, profit_matrix, num))
                asyncio.create_task(self.shot_pic_org(self.current_state, epoch, self.r, profit_matrix, num))
            ...
            if (total_epoch+1 in [1, 10, 100, 1000, 10000, 100000]):
                profit_matrix = self.calculate_reward(self.current_state)
                asyncio.create_task(self.shot_pic(self.current_state, total_epoch+1, self.r, profit_matrix, num))
            if (epoch+1 in [1, 10, 100, 1000, 10000, 100000]):
                profit_matrix = self.calculate_reward(self.current_state)
                asyncio.create_task(self.shot_pic_org(self.current_state, epoch+1, self.r, profit_matrix, num))
    ...
    async def shot_pic_org(self, type_t_matrix, epoch, r, profit_matrix, run_num):
        """保存策略矩阵快照与数据文件（与原Q-learning代码相同格式）"""
        plt.clf()
        plt.close("all")
        
        # 创建输出目录
        img_dir = f'{self.output_path}/shot_pic/org/r={r}/two_type_{run_num}'
        matrix_dir = f'{self.output_path}/shot_pic/org/r={r}/two_type_{run_num}/type_t_matrix'
        profit_dir = f'{self.output_path}/shot_pic/org/r={r}/two_type_{run_num}/profit_matrix'
    ...
    def choose_action(self, state_matrix,t):
        with torch.no_grad():
            features = self.encode_state(state_matrix)
            probs, _ = self.policy(features,t)
            dist = Categorical(probs)
            actions = dist.sample()
